...
class Mario(pg.sprite.Sprite):
    ...
    def setup_state_booleans(self):
        #set up booleans that affect behavior
    ...
    def setup_forces(self):
        #set up forces that affect Mario's velocity
        self.x_vel = 0
        self.y_vel = 0
    ...
    def setup_counters(self):
        #keep track of totals for values
        self.frame_index = 0
    ...
    def load_images_from_sheet(self):
        #get Mario images from sprite sheet and assigns them 
        self.right_frames = []
        self.left_frames = []
        ...
        self.all_images = [self.right_big_normal_frames,self.right_big_black_frames,
                           self.right_big_red_frames,self.right_big_green_frames,
                           self.right_small_normal_frames,self.right_small_green_frames,
                           self.right_small_red_frames,self.right_small_black_frames,
                           self.left_big_normal_frames,self.left_big_black_frames,
                           self.left_big_red_frames,self.left_big_green_frames,
                           self.left_small_normal_frames,self.left_small_red_frames,
                           self.left_small_green_frames,self.left_small_black_frames]
        self.right_frames = self.normal_small_frames[0]
        self.left_frames = self.normal_small_frames[1]

    def get_image(self, x, y, width, height):
        #gets image from sprite 
    ...
    def update(self, keys, game_info, fire_group):
        #updates Mario's states and animations per frame
    ...
    def handle_state(self, keys, fire_group):
        #determines Mario's behavior based on state
    ...
    def standing(self, keys, fire_group):
        #call if Mario is standing still
    ...
    def count_number_of_fireballs(self, powerup_group):
        #ount number of fireballs that exist in the level
        fireball_list = []
    ...
    def walking(self, keys, fire_group):
        #call when Mario is walking, changes the frame, checks for holding down the run button,
        #checks for a jump, then adjusts the state 
        self.check_to_allow_jump(keys)
        self.check_to_allow_fireball(keys)
    ...
    def calculate_animation_speed(self):
        #make walking animation speed be in relation to Mario's x-vel
        if self.x_vel == 0:
            animation_speed = 130
        elif self.x_vel > 0:
    ...
    def jumping(self, keys, fire_group):
        #call when Mario is in a JUMP state
    ...
    def falling(self, keys, fire_group):
        #call when Mario is in falling state
        self.check_to_allow_fireball(keys)
    ...
    def jumping_to_death(self):
        #call when Mario is in a DEATH_JUMP state
    ...
    def start_death_jump(self, game_info):
        #use to put Mario in a DEATH_JUMP state
        self.dead = True
    ...
    def changing_to_fire(self):
        #call when Mario is in a BIG_TO_FIRE state 
        self.in_transition_state = True
        if self.facing_right:
            frames = [self.right_fire_frames[3],self.right_big_green_frames[3],
                      self.right_big_red_frames[3],self.right_big_black_frames[3]]
        else:
            frames = [self.left_fire_frames[3],self.left_big_green_frames[3],
                      self.left_big_red_frames[3],self.left_big_black_frames[3]]
    ...
    def changing_to_small(self):
        ...
        if self.facing_right:
            frames = [self.right_big_normal_frames[4],self.right_big_normal_frames[8],
                      self.right_small_normal_frames[8]]
        else:
            frames = [self.left_big_normal_frames[4],self.left_big_normal_frames[8],
    ...
    def adjust_rect(self):
        #make sure new Rect has the same bottom and left location as previous one
        x = self.rect.x
        bottom = self.rect.bottom
        self.rect = self.image.get_rect()
        self.rect.x = x
    ...
    def flag_pole_sliding(self):
        #Mario sliding down the flag pole
    ...
    def check_for_special_state(self):
        #determines if Mario is invincible or fire Mario
    ...
    def check_if_hurt_invincible(self):
        #check if Mario is still temporarily invincible after getting injured
