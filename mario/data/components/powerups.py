...
class Powerup(pg.sprite.Sprite):
    #base class for powerup group
    def __init__(self, x, y):
        super(Powerup, self).__init__()

    def setup_powerup(self, x, y, name, setup_frames):
        #separate setup function allows to pass a different setup_frames method depending on powerup
    ...
    def get_image(self, x, y, width, height):
        #Get image frames from the sprite 
    ...
    def revealing(self, *args):
        #powerup leaves the coin box or brick
    ...
    def sliding(self):
        #powerup slides along the ground
    ...
    def falling(self):
        #powerups fall of a ledge
        if self.y_vel < self.max_y_vel:
            self.y_vel += self.gravity

class Mushroom(Powerup):
    #Powerup that makes Mario get bigger
...
class FireFlower(Powerup):
    #allow Mario to throw fire balls
...
class Star(Powerup):
    ...
    def start_bounce(self, vel):
        #transition into bouncing state
        self.y_vel = vel

    def bouncing(self):
        #star is bouncing around
        self.animation()
...
class FireBall(pg.sprite.Sprite):
    ...
    def get_image(self, x, y, width, height):
        #get the image frames from the sprite 
        image = pg.Surface([width, height]).convert()
        rect = image.get_rect()
        image.blit(self.sprite_sheet, (0, 0), (x, y, width, height))
        image.set_colorkey(c.BLACK)
